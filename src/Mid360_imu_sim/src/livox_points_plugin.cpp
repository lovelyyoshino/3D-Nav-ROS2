...
void convertDataToRotateInfo(const std::vector<std::vector<double>> &datas, std::vector<AviaRotateInfo> &avia_infos) {
    ...
    for (auto &data : datas) {
        if (data.size() == 3) {
            ...
            RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "data size is not 3!");
        }
    }
}
...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "load csv file name:" << csv_file_name);
    }
    std::string resolved_csv_file_name = sdf::findFile(csv_file_name, true, true);
    if (resolved_csv_file_name.empty())
    {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "cannot resolve csv file!" << csv_file_name << "will return !");
        return;
    }   
    if (!CsvReader::ReadCsvFile(resolved_csv_file_name, datas)) {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "cannot get csv file!" << resolved_csv_file_name << "will return !");
        return;
    }
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "ros topic name:" << curr_scan_topic);
    if (!rclcpp::ok()) {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "scan info size:" << aviaInfos.size());
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "sample:" << samplesStep);
    RCLCPP_INFO_STREAM(rclcpp::get_logger("livox_points_plugin"), "downsample:" << downSample);
...
void LivoxPointsPlugin::SendRosTf(const ignition::math::Pose3d &pose, const std::string &father_frame,
    ...
    tf_msg.header.stamp = rosNode->now();
    tf_msg.header.frame_id = father_frame;
