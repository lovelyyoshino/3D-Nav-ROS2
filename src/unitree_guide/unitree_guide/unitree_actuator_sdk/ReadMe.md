# Introduction
This library is used for the communication between PC and the motor control board, so that the user can control the motors by PC. This library includes Linux and Windows version, where the Linux version also support ROS2. And we offer the usage examples of C, C++ and Python.
...
## /unitree_motor_ctrl
It is a package of ROS2 and the library file is just the library file of Linux.
## /build
Build directory
## /bin
...
## C/C++ under ROS2
### Build
Under the colcon workspace, run:
```bash
colcon build
```
### Run
...
ros2 run unitree_motor_ctrl unitree_motor_ctrl_node
```
## Python under ROS2
In a terminal, run:
```bash
sudo su
source install/setup.bash
ros2 run unitree_motor_ctrl check.py
```
## C/C++ under Windows
### Build
