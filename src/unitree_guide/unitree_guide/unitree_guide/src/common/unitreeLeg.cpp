...
Vec3 QuadrupedLeg::calcQ(Vec3 pEe, FrameType frame){
    ...
    float b_sq = pow(c, 2) - pow(a, 2);
    if(b_sq < 0) b_sq = 0;  // clamp to avoid NaN when target is inside abad sphere
    b = sqrt(b_sq); // distance between shoulder and footpoint
...
Vec3 QuadrupedLeg::calcQd(Vec3 q, Vec3 vEe){
    ...
    if(std::fabs(det) < 1e-6f){
        // Near singularity: use damped least squares (damped pseudoinverse)
        float lambda = 0.01f;
        return J.transpose() * (J * J.transpose() + lambda * lambda * Mat3::Identity()).inverse() * vEe;
    }
    return J.inverse() * vEe;
}
...
float QuadrupedLeg::q1_ik(float py, float pz, float l1){
    float q1;
    float L_sq = pow(py,2)+pow(pz,2)-pow(l1,2);
    if(L_sq < 0) L_sq = 0;  // clamp to avoid NaN when target is too close to hip
    float L = sqrt(L_sq);
