...
void BalanceCtrl::solveQP(){
    ...
    if(std::isinf(value) || std::isnan(value)){
        // QP infeasible or numerical failure: zero out forces to prevent NaN propagation
        std::cerr << "[BalanceCtrl] QP solver infeasible (value=" << value << "), zeroing forces" << std::endl;
        _F.setZero();
        return;
    }
