...
The interface between ROS2 and real robot: `unitree_legged_real`

## Environment
We recommand users to run this package in Ubuntu 22.04 and ROS2 Humble environment.
...
# Configuration
Before compiling this package, please download the corresponding unitree_legged_sdk as noted above, and put it to your own workspace's source folder(e.g. `~/ros2_ws/src`). Be careful with the sdk folder name. It should be "unitree_legged_sdk" without version tag.

# Build
You can use colcon to build ROS2 packages. First copy the package folder to `~/ros2_ws/src`, then:
```
cd ~/ros2_ws
colcon build
```
...
# Run the package
You can control your real Go1 robot from ROS2 by this package.

Before you run expamle program, please run command

```
ros2 launch unitree_legged_real real.launch.py ctrl_level:=highlevel
```
or
```
ros2 launch unitree_legged_real real.launch.py ctrl_level:=lowlevel
```
