...
UNITREE_LEGGED_SDK::HighCmd rosMsg2Cmd(const unitree_legged_msgs::msg::HighCmd::SharedPtr &msg)
{
...
unitree_legged_msgs::msg::HighState state2rosMsg(UNITREE_LEGGED_SDK::HighState &state)
{
    ...
    for (int i(0); i < 4; i++)
    {
        ...
        ros_msg.foot_position2_body[i] = state2rosMsg(state.footPosition2Body[i]);
        ros_msg.foot_speed2_body[i] = state2rosMsg(state.footSpeed2Body[i]);
    }
